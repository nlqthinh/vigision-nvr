...
from vigision.networks.linear_dense_stgcn import Lin_DenseSTGCN
from vigision.networks.oneshot_stgcn_nano import OSA_STGCN_nano_1S
from vigision.pose_utils import normalize_points_with_size, scale_pose
...
class TSSTG(object):
    ...
    def __init__(self,
        ...
        self.model = OSA_STGCN_nano_1S(num_class = self.num_class, graph_args = self.graph_args).to(self.device)
        self.model.load_state_dict(torch.load(weight_file, map_location=torch.device(device), weights_only=True), strict=False)
        self.model.eval()
