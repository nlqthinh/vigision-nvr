...
from vigision.comms.dispatcher import Dispatcher
from vigision.comms.events_updater import EventEndSubscriber, EventUpdatePublisher
from vigision.comms.inter_process import InterProcessRequestor
from vigision.config import (
    CameraConfig,
    VigisionConfig,
    MqttConfig,
    RecordConfig,
...
from vigision.const import ALL_ATTRIBUTE_LABELS, CLIPS_DIR, UPDATE_CAMERA_ACTIVITY
from vigision.events.types import EventStateEnum, EventTypeEnum
...
from vigision.util.object import box_inside
...
class TrackedObject:
    ...
    def to_dict(self, include_thumbnail: bool = False):
        event = {
            ...
            "estimated_box": self.obj_data["estimated_box"],
            "attributes": self.attributes,
            "current_attributes": self.obj_data["attributes"],
            "fall_data": self.obj_data.get("fall_data"),
        }
    ...
    def get_current_frame(self, draw_options={}):
        ...
        if draw_options.get("bounding_boxes"):
            ...
            for obj in tracked_objects.values():
                ...
                if not (obj["frame_time"] != frame_time and not (box_inside((3, 3, frame_shape[1] - 3, frame_shape[0] - 3), (box[0], box[1], box[2], box[3])))):
                    draw_box_with_label(
                        frame_copy,
                        box[0],
                        box[1],
                        ...
                        f"{obj['score']:.0%}" if frame_time == obj["frame_time"] else "",
                        thickness=thickness,
                        color=color,
                    )
        ...
        # if self.fall_data:
        if draw_options.get("fall_detect") or draw_options.get("pose_estimation"):
            for obj in tracked_objects.values():
                if (obj["fall_data"] is not None):
                    label = obj["fall_data"]['label']
                    if (label == 'Non-fall' or label == 'Fall'):
                        score = obj["fall_data"]['score']
                        text = f'{label}: {score * 100:.2f}%'
                        bbox = obj["fall_data"]['box']
                        if label == 'Non-fall':
                        ...
                        text_x = bbox[0]
                        text_y = bbox[1]-30   
                        if draw_options.get("fall_detect"):
                            if not (obj["frame_time"] != frame_time and not (box_inside((3, 3, frame_copy.shape[1] - 3, frame_copy.shape[0] - 3), (bbox[0], bbox[1], bbox[2], bbox[3])))):
                                cv2.rectangle(frame_copy, (text_x, text_y - text_size[1] - padding), (text_x + text_size[0] + padding, text_y + padding), text_background_color, -1)
                                cv2.putText(frame_copy, label, (text_x, text_y), font, font_scale, text_color, font_thickness, cv2.LINE_AA)
                        if draw_options.get("pose_estimation"):
                            if not (obj["frame_time"] != frame_time and not (box_inside((3, 3, frame_copy.shape[1] - 3, frame_copy.shape[0] - 3), (bbox[0], bbox[1], bbox[2], bbox[3])))):
                                frame_copy = draw_single(frame_copy, np.array(obj["fall_data"]['pose']))
