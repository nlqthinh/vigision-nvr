...
from vigision.config import DetectConfig, ModelConfig
...
from vigision.detectors.detector_config import BaseDetectorConfig, PixelFormatEnum
from vigision.models import Event, Regions, Timeline
...
# def create_tensor_input(frame, model_config: ModelConfig):

#     # Assuming frame is already in RGB format; if not, convert it
#     rgb_frame = yuv_to_bgr(frame) 

#     # Resize if needed
#     if rgb_frame.shape != (model_config.height, model_config.width, 3):
...
def create_tensor_input(frame, model_config: ModelConfig, detector_config: BaseDetectorConfig, region):
    if detector_config["detector_name"].type == "cpu":
        cropped_frame = yuv_region_2_rgb(frame, region)
    elif detector_config["detector_name"].type == "gpu":
