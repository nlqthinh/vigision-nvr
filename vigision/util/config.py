...
from vigision.const import CONFIG_DIR, EXPORT_DIR
from vigision.util.services import get_video_properties
...
def migrate_vigision_config(config_file: str):
    """handle migrating the vigision config."""
    logger.info("Checking if vigision config needs migration...")
    ...
    if previous_version == CURRENT_CONFIG_VERSION:
        logger.info("vigision config does not need migration...")
        return
    ...
    if previous_version < 0.14:
        logger.info(f"Migrating vigision config from {previous_version} to 0.14...")
    ...
    logger.info("Finished vigision config migration...")


def migrate_014(config: dict[str, dict[str, any]]) -> dict[str, dict[str, any]]:
    """Handle migrating vigision config to 0.14"""
