...
from vigision.config import BirdseyeModeEnum, VigisionConfig
from vigision.const import MODEL_CACHE_DIR
from vigision.detectors import DetectorTypeEnum
from vigision.plus import PlusApi
from vigision.util.builtin import deep_merge, load_config_with_no_duplicates
...
class TestConfig(unittest.TestCase):
    ...
    def test_config_class(self):
        vigision_config = VigisionConfig(**self.minimal)
        assert self.minimal == vigision_config.model_dump(exclude_unset=True)

        runtime_config = vigision_config.runtime_config()
    ...
    @patch("vigision.detectors.detector_config.load_labels")
    def test_detector_custom_model_path(self, mock_labels):
        ...
        vigision_config = VigisionConfig(**(deep_merge(config, self.minimal)))
        runtime_config = vigision_config.runtime_config()
    ...
    def test_invalid_mqtt_config(self):
        ...
        self.assertRaises(ValidationError, lambda: VigisionConfig(**config))
    ...
    def test_inherit_tracked_objects(self):
        ...
        vigision_config = VigisionConfig(**config)
        assert config == vigision_config.model_dump(exclude_unset=True)
    ...
    def test_motion_mask_relative_matches_explicit(self):
        ...
        vigision_config = VigisionConfig(**config).runtime_config()
        assert np.array_equal(
            vigision_config.cameras["explicit"].motion.mask,
            vigision_config.cameras["relative"].motion.mask,
        )
    ...
    def test_zone_relative_matches_explicit(self):
        ...
        vigision_config = VigisionConfig(**config).runtime_config()
        assert np.array_equal(
            vigision_config.cameras["back"].zones["explicit"].contour,
            vigision_config.cameras["back"].zones["relative"].contour,
        )
    ...
    def test_plus_labelmap(self):
        ...
        runtime_config = vigision_config.runtime_config(PlusApi())
    ...
    def test_fails_on_missing_role(self):
        ...
        vigision_config = VigisionConfig(**config)
        self.assertRaises(ValueError, lambda: vigision_config.runtime_config())
    ...
    def test_works_on_missing_role_multiple_cams(self):
        ...
        vigision_config = VigisionConfig(**config)
        vigision_config.runtime_config()
    ...
    def test_fails_on_bad_camera_name(self):
        ...
        self.assertRaises(
            ValidationError, lambda: vigision_config.runtime_config().cameras
        )
    ...
    def test_fails_on_bad_segment_time(self):
        ...
        self.assertRaises(
            ValueError,
            lambda: vigision_config.runtime_config().ffmpeg.output_args.record,
        )
    ...
    def test_fails_zone_defines_untracked_object(self):
        ...
        self.assertRaises(ValueError, lambda: vigision_config.runtime_config().cameras)
    ...
    def test_fails_invalid_movement_weights(self):
        ...
        self.assertRaises(ValueError, lambda: VigisionConfig(**config))
