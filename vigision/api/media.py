...
from vigision.models import Event, Previews, Recordings, Regions, ReviewSegment
from vigision.util.builtin import get_tz_modifiers
from vigision.util.image import get_image_from_recording
...
def mjpeg_feed(camera_name):
    ...
    draw_options = {
        ...
        "pose_estimation": request.args.get("pose_estimation", type=int),
        "fall_detect": request.args.get("fall_detect", type=int),
    }
    if camera_name in current_app.vigision_config.cameras:
        # return a multipart response
        return Response(
            imagestream(
                current_app.detected_frames_processor,
                current_app.camera_error_image,
                camera_name,
                fps,
                height,
                draw_options,
...
def imagestream(detected_frames_processor, camera_error_image, camera_name, fps, height, draw_options):
    while True:
        # max out at specified FPS
        time.sleep(1 / fps)
        frame = detected_frames_processor.get_current_frame(camera_name, draw_options)
        if frame is None or datetime.now().timestamp() > (
            detected_frames_processor.get_current_frame_time(camera_name)
            + 10
        ):
            if camera_error_image is None:
                error_image = glob.glob("/opt/vigision/vigision/images/camera-error.jpg")

                if len(error_image) > 0:
                    camera_error_image = cv2.imread(
                        error_image[0], cv2.IMREAD_UNCHANGED
                    )
            frame = camera_error_image
...
def latest_frame(camera_name):
    ...
    if camera_name in current_app.vigision_config.cameras:
        ...
        retry_interval = float(
            current_app.vigision_config.cameras.get(camera_name).ffmpeg.retry_interval
            or 10
        )
        ...
        return response
    elif camera_name == "birdseye" and current_app.vigision_config.birdseye.restream:
        frame = cv2.cvtColor(
...
def get_snapshot_from_recording(camera_name: str, frame_time: str):
    if camera_name not in current_app.vigision_config.cameras:
        return make_response(
            jsonify({"success": False, "message": "Camera not found"}),
            404,
        )
...
def grid_snapshot(camera_name):
    ...
    if camera_name in current_app.vigision_config.cameras:
        detect = current_app.vigision_config.cameras[camera_name].detect
...
def event_snapshot_clean(id):
    ...
    try:
        ...
        snapshot_config = current_app.vigision_config.cameras[event.camera].snapshots
