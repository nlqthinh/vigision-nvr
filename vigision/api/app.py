...
from vigision.api.auth import AuthBp, get_jwt_secret, limiter
from vigision.api.event import EventBp
from vigision.api.export import ExportBp
from vigision.api.media import MediaBp
from vigision.api.preview import PreviewBp
from vigision.api.review import ReviewBp
from vigision.config import VigisionConfig
from vigision.const import CONFIG_DIR
from vigision.events.external import ExternalEventProcessor
from vigision.models import Event, Timeline
...
from vigision.util.services import ffprobe_stream, restart_vigision, vainfo_hwaccel
...
bp = Blueprint("vigision", __name__)
...
def create_app(
    ...
    app.config['MAIL_USERNAME'] = 'thinhnlqce161130@fpt.edu.vn'
    app.config['MAIL_PASSWORD'] = 'lguzeixyikhmobwx'
    ...
    app.vigision_config = vigision_config
    ...
    app.jwt_token = get_jwt_secret() if vigision_config.auth.enabled else None
    # update the request_address with the x-forwarded-for header from nginx
    app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1)
    # initialize the rate limiter for the login endpoint
    limiter.init_app(app)
    if vigision_config.auth.failed_login_rate_limit is None:
...
@bp.route("/")
def is_healthy():
    return "Vigision is running. Alive and healthy!"


@bp.route("/config/schema.json")
def config_schema():
    return current_app.response_class(
        current_app.vigision_config.schema_json(), mimetype="application/json"
    )


@bp.route("/gpu_available", methods=["GET"])
def gpu_available():
    try:
        gpu_status = torch.cuda.is_available()
        return jsonify({"gpu_available": gpu_status}), 200
    except Exception as e:
        logger.error(f"Error checking GPU availability: {e}")
        return make_response(
            jsonify({"success": False, "message": "Error checking GPU availability"}), 500
        )
...
@bp.route("/get_all_labels")
def get_all_labels():
    return jsonify(current_app.vigision_config.model.merged_labelmap)

@bp.route("/config")
def config():
    config_obj: VigisionConfig = current_app.vigision_config
    ...
    for camera_name, camera in current_app.vigision_config.cameras.items():
    ...
    for detector_config in config["detectors"].values():
        detector_config["model"]["labelmap"] = (
            current_app.vigision_config.model.merged_labelmap
        )
...
def config_save():
    ...
    try:
        VigisionConfig.parse_raw(new_config)
    except Exception:
        return make_response(
            jsonify(
                {
    ...
    try:
        ...
        return make_response(
            jsonify(
                {
                    "success": False,
                    "message": "Could not write config file, be sure that Vigision has write permission on the config file.",
                }
            ),
            400,
        )
    ...
    if save_option == "restart":
        try:
            ...
            logging.error(f"Error restarting Vigision: {e}")
            return make_response(
                jsonify(
                    {
                        "success": True,
                        "message": "Config successfully saved, unable to restart Vigision",
                    }
                ),
                200,
            )
...
def config_set():
    ...
    try:
        ...
        # Validate the config schema
        try:
            config_obj = VigisionConfig.parse_raw(new_raw_config)
        except Exception:
            with open(config_file, "w") as f:
    ...
    if json.get("requires_restart", 1) == 0:
        current_app.vigision_config = VigisionConfig.runtime_config(
...
def ffprobe():
    ...
    if path_param.startswith("camera"):
        ...
        if camera not in current_app.vigision_config.cameras.keys():
            return make_response(
                jsonify(
        ...
        if not current_app.vigision_config.cameras[camera].enabled:
            return make_response(
        ...
        paths = map(
            ...
            current_app.vigision_config.cameras[camera].ffmpeg.inputs,
        )
...
def logs(service: str):
    log_locations = {
        "vigision": "/dev/shm/logs/vigision/current",
    ...
    try:
        ...
        for rawLine in contents.splitlines():
            ...
            if dateEnd == 0:
                ...
                keyLength = dateEnd - (6 if service_location == "vigision" else 0)
    ...
    try:
        ...
        return make_response(
            jsonify(
                {
                    "success": False,
                    "message": "Unable to restart Vigision.",
                }
            ),
            500,
        )
