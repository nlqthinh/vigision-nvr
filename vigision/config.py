...
from vigision.detectors import DetectorConfig, ModelConfig
from vigision.detectors.detector_config import BaseDetectorConfig
from vigision.ffmpeg_presets import (
...
from vigision.util.builtin import (
    deep_merge,
...
from vigision.util.config import StreamInfoRetriever, get_relative_coordinates
from vigision.util.image import create_mask
from vigision.util.services import auto_detect_hwaccel
...
if os.path.isdir("/run/secrets") and os.access("/run/secrets", os.R_OK):
    for secret_file in os.listdir("/run/secrets"):
        if secret_file.startswith("VIGISION_"):
            VIGISION_ENV_VARS[secret_file] = (
                Path(os.path.join("/run/secrets", secret_file)).read_text().strip()
            )

DEFAULT_TRACKED_OBJECTS = ["person"]
DEFAULT_ALERT_OBJECTS = ["person", "car", "motorcycle"]
...
class VigisionBaseModel(BaseModel):
...
class UIConfig(VigisionBaseModel):
...
class AuthConfig(VigisionBaseModel):
    ...
    cookie_name: str = Field(
        default="vigision_token", title="Name for jwt token cookie", pattern=r"^[a-z]_*$"
    )
    cookie_secure: bool = Field(default=False, title="Set secure flag on cookie")
    session_length: int = Field(
        default=3600, title="Session length for jwt session tokens", ge=60
    )
...
class StatsConfig(VigisionBaseModel):
...
class TelemetryConfig(VigisionBaseModel):
...
class MqttConfig(VigisionBaseModel):
    enabled: bool = Field(title="Enable MQTT Communication.", default=False)
    host: str = Field(default="", title="MQTT Host")
    port: int = Field(default=1883, title="MQTT Port")
    topic_prefix: str = Field(default="vigision", title="MQTT Topic Prefix")
    client_id: str = Field(default="vigision", title="MQTT Client ID")
...
class PtzAutotrackConfig(VigisionBaseModel):
    enabled: bool = Field(default=False, title="Enable PTZ object autotracking.")
    calibrate_on_startup: bool = Field(
        default=False, title="Perform a camera calibration when Vigision starts."
    )
...
class OnvifConfig(VigisionBaseModel):
...
class RetainConfig(VigisionBaseModel):
...
class EventsConfig(VigisionBaseModel):
...
class RecordRetainConfig(VigisionBaseModel):
    days: float = Field(default=0, title="Default retention period.")
    mode: RetainModeEnum = Field(default=RetainModeEnum.all, title="Retain mode.")


class RecordExportConfig(VigisionBaseModel):
...
class MotionConfig(VigisionBaseModel):
...
class StationaryMaxFramesConfig(VigisionBaseModel):
...
class StationaryConfig(VigisionBaseModel):
...
class DetectConfig(VigisionBaseModel):
...
class FallDetectConfig(VigisionBaseModel):
    enabled: bool = Field(default=True, title="Fall Detection Enabled.")



class FilterConfig(VigisionBaseModel):
    min_area: int = Field(
...
class AudioFilterConfig(VigisionBaseModel):
    threshold: float = Field(
        default=0.8,
...
class DetectionsConfig(VigisionBaseModel):
...
class ReviewConfig(VigisionBaseModel):
    """Configure reviews"""
...
class AudioConfig(VigisionBaseModel):
...
class BirdseyeLayoutConfig(VigisionBaseModel):
...
class BirdseyeConfig(VigisionBaseModel):
...
class FfmpegOutputArgsConfig(VigisionBaseModel):
...
class FfmpegConfig(VigisionBaseModel):
...
class CameraInput(VigisionBaseModel):
...
class SnapshotsConfig(VigisionBaseModel):
...
class ColorConfig(VigisionBaseModel):
...
class TimestampStyleConfig(VigisionBaseModel):
...
class CameraMqttConfig(VigisionBaseModel):
...
class CameraLiveConfig(VigisionBaseModel):
...
class CameraUiConfig(VigisionBaseModel):
    order: int = Field(default=0, title="Order of camera in UI.")
    dashboard: bool = Field(
        default=True, title="Show this camera in Vigision dashboard UI."
    )


class CameraConfig(VigisionBaseModel):
    name: Optional[str] = Field(None, title="Camera name.", pattern=REGEX_CAMERA_NAME)
    display_name: Optional[str] = Field(None, title="Camera display name.", pattern=REGEX_CAMERA_NAME)
    ...
    fall_detect: FallDetectConfig = Field(
        default_factory=FallDetectConfig, title="Fall detection configuration."
    )
...
class DatabaseConfig(VigisionBaseModel):
...
class LoggerConfig(VigisionBaseModel):
...
class CameraGroupConfig(VigisionBaseModel):
    """Represents a group of cameras."""

    cameras: Union[str, List[str]] = Field(
        default_factory=list, title="List of cameras in this group."
    )
    display_name: str = Field(default="Group name", title="Camera group display name.", pattern=REGEX_CAMERA_NAME) 
...
def verify_valid_live_stream_name(
    vigision_config: VigisionConfig, camera_config: CameraConfig
) -> ValueError | None:
    """Verify that a restream exists to use for live view."""
    if (
        camera_config.live.stream_name
        not in vigision_config.go2rtc.model_dump().get("streams", {}).keys()
    ):
        return ValueError(
...
class VigisionConfig(VigisionBaseModel):
    mqtt: MqttConfig = Field(default_factory=MqttConfig, title="MQTT configuration.")
    ...
    environment_vars: Dict[str, str] = Field(
        default_factory=dict, title="Vigision environment variables."
    )
    ...
    fall_detect: FallDetectConfig = Field(
        default_factory=FallDetectConfig, title="Global fall detection configuration."
    )
    cameras: Dict[str, CameraConfig] = Field(
        default_factory=dict, title="Camera configuration."
    )    
    ...
    def runtime_config(self, plus_api: PlusApi = None) -> VigisionConfig:
        ...
        if config.mqtt.user or config.mqtt.password:
            config.mqtt.user = config.mqtt.user.format(**VIGISION_ENV_VARS)
            config.mqtt.password = config.mqtt.password.format(**VIGISION_ENV_VARS)
        ...
        for name, camera in config.cameras.items():
            ...
            for input in camera_config.ffmpeg.inputs:
                input.path = input.path.format(**VIGISION_ENV_VARS)
