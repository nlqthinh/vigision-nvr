...
import { VigisionStats } from "@/types/stats";
import { useEffect, useMemo, useState } from "react";
import { useVigisionStats } from "@/api/ws";
import {
...
export default function GeneralMetrics({
  ...
  const { data: initialStats } = useSWR<VigisionStats[]>(
    [
      "stats/history",
  ...
  const [statsHistory, setStatsHistory] = useState<VigisionStats[]>([]);
  const { payload: updatedStats } = useVigisionStats();
  ...
  const otherProcessMemSeries = useMemo(() => {
    ...
    statsHistory.forEach((stats, statsIdx) => {
      ...
      Object.entries(stats.processes).forEach(([key, procStats]) => {
        // if key is not go2rtc and logger
        if (key != "go2rtc" && key != "logger") {
          if (procStats.pid.toString() in stats.cpu_usages) {
            if (!(key in series)) {
              series[key] = { name: key, data: [] };
  ...
  return (
    <>
      ...
      <div className="scrollbar-container mt-4 flex size-full flex-col overflow-y-auto px-2">
        ...
        >
          {statsHistory.length != 0 ? (
            <div className="rounded-lg bg-background_alt p-2.5 md:rounded-2xl">
              <div className="mb-2">Detector Inference Speed</div>
        ...
        {(statsHistory.length == 0 || statsHistory[0].gpu_usages) && (
          <>
            ...
            <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
              {statsHistory.length != 0 ? (
                <div className="rounded-lg bg-background_alt p-2.5 md:rounded-2xl">
                  <div className="mb-2">GPU Usage</div>
              ...
              {statsHistory.length != 0 ? (
                <>
                  {gpuMemSeries && (
                    <div className="rounded-lg bg-background_alt p-2.5 md:rounded-2xl">
                      <div className="mb-2">GPU Memory</div>
