...
import { VigisionStats, PotentialProblem } from "@/types/stats";
...
export default function useStats(stats: VigisionStats | undefined) {
...
export function useAutoVigisionStats() {
  const { data: initialStats } = useSWR<VigisionStats>("stats", {
    revalidateOnFocus: false,
  });
  const { payload: latestStats } = useVigisionStats();
