...
function useHelpfulLinks(content: string | undefined) {
  return useMemo(() => {
    ...
    if (/Could not clear [\d.]* currently [\d.]*/.exec(content)) {
      links.push({
        link: "https://docs.vigision.video/configuration/record#will-vigision-delete-old-recordings-if-my-storage-runs-out",
        text: "Vigision Automatic Storage Cleanup",
      });
    }

    if (/Did not detect hwaccel/.exec(content)) {
      links.push({
        link: "https://docs.vigision.video/configuration/hardware_acceleration",
    ...
    if (content.includes("No EdgeTPU was detected")) {
      links.push({
        link: "https://docs.vigision.video/troubleshooting/edgetpu",
    ...
    if (content.includes("The current SHM size of")) {
      links.push({
        link: "https://docs.vigision.video/vigision/installation/#calculating-required-shm-size",
