...
export function LiveGridIcon({ layout }: LiveIconProps) {
  return (
    <div className="flex size-full flex-col gap-0.5 overflow-hidden rounded-md">
      <div
        className={`h-1 w-full ${layout == "grid" ? "bg-green-400" : "bg-muted-foreground"}`}
      />
      <div className="flex h-1 w-full gap-0.5">
        <div
          className={`w-full ${layout == "grid" ? "bg-green-400" : "bg-muted-foreground"}`}
        />
        <div
          className={`w-full ${layout == "grid" ? "bg-green-400" : "bg-muted-foreground"}`}
        />
...
export function LiveListIcon({ layout }: LiveIconProps) {
  return (
    <div className="flex size-full flex-col gap-0.5 overflow-hidden rounded-md">
      <div
        className={`size-full ${layout == "list" ? "bg-green-400" : "bg-secondary-foreground"}`}
      />
      <div
        className={`size-full ${layout == "list" ? "bg-green-400" : "bg-secondary-foreground"}`}
      />
