...
import { ATTRIBUTE_LABELS, VigisionConfig } from "@/types/vigisionConfig";
...
export default function ObjectMaskEditPane({
        ...
        .then((res) => {
          if (res.status === 200) {
            toast.success(
              ...
              <p>{`${polygon.name || "Object Mask"} has been saved. Restart Vigision to apply changes.`}</p>
              <Button
                size="sm"
                onClick={() => {
  ...
  useEffect(() => {
    document.title = "Edit Object Mask - Vigision";
  }, []);
    ...
    <>
      ...
      <Heading as="h3" className="my-2">
        ...
        <Tooltip>
          ...
          <TooltipContent side="bottom" className="z-50 mr-2">
            <div className="my-2 flex flex-col gap-2 text-sm text-primary-variant">
            <p>
              Object filter masks are used to filter out false positives <br /> for a given
