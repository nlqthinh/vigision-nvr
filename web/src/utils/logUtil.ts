...
const vigisionDateStamp = /\[[\d\s-:]*]/;
const vigisionSeverity = /(DEBUG)|(INFO)|(WARNING)|(ERROR)/;
const vigisionSection = /[\w.]*/;
...
export function parseLogLines(logService: LogType, logs: string[]) {
  if (logService == "vigision") {
    return logs
      .map((line) => {
        const match = vigisionDateStamp.exec(line);
        ...
        const sectionMatch = vigisionSection.exec(
    ...
    return logs
      .map((line) => {
        ...
        if (vigisionSeverity.exec(section)) {
          section = "startup";
        }
